...
const HELP_CATEGORIES = [
    {
        ...
        lines: [
            ...
            '1️⃣ `/setup-verify role:@Verified` — verification panel · `/set-role unverified @Unverified` — new-member marker (optional)',
            '2️⃣ `/add-category` + `/add-product` — prepare the catalog',
            '3️⃣ `/setup-ticket-panel` — mount the ticket panel',
            '4️⃣ `/setup-selfrole` — self-role panels · `/add-responder` — FAQ auto-reply',
        ...
        detail: [
            ...
            '1️⃣ `/setup-verify role:@Verified` — the verification panel · `/set-role tipe:unverified role:@Unverified` (optional) — new members are marked Unverified on join and the marker disappears automatically when they verify',
            ...
            '4️⃣ `/setup-selfrole` — other self-role panels (roles members pick themselves)',
    ...
    },
    {
        ...
        detail: [
            ...
            '',
            '**Member verification (v4.2.0 — classic feature restored)**',
            '• `/setup-verify role:@Verified` — the verification panel: members click the button to get the Verified role.',
            '• `/set-role tipe:unverified role:@Unverified` (v4.3.0 — back) — the new-member marker: granted automatically on join, removed automatically on verify.',
    ...
    },
    {
        ...
        lines: [
            ...
            '• `/set-role admin/midman/**booster** @role` — system roles (+ `verified` / `unverified`, v4.3.0)',
        ...
        detail: [
            ...
            '• New-member marker: `/set-role tipe:unverified role:@Unverified` (v4.3.0 — back) — granted automatically on join, removed automatically the moment the member verifies.',
            '',
            '**System roles (bot logic)** — `/set-role tipe role`:',
            '• `tipe:admin` — who may use admin commands · `tipe:verified` (v4.2.0 — back) — the verification role · `tipe:unverified` (v4.3.0 — back) — the new-member marker, held until verify · `tipe:midman` — escrow officers · `tipe:booster` — auto boost role. `/remove-role tipe` clears one.',
            '• `tipe:unverified` (v4.3.0) — the classic CHRONOS new-member marker: granted automatically on join, removed automatically by the verify click. Pair it with `/setup-verify` for the full chain.',
