...
12. [Public Mode + Developer Portal](#12-public-mode--developer-portal-phase-3)
...
- In Discord, type `/` — all **96 slash commands** must appear
- If a command doesn't show up, make sure `GUILD_ID` in `.env` is correct

> 🌍 **ONE GUILD ID (v3.12.0)** — all server behavior is controlled by ONE line in `.env`: `GUILD_ID`. **Set** = single-server mode (default): slash commands register **instantly** to that server, and events from other servers (messages/joins/boosts/tickets) are ignored — insurance in case the bot is accidentally invited. **Empty** = **public mode**: global commands (appear automatically in every server that invites the bot, ~1 hour), **per-server** configs at `data/config/<guildId>.json` — server A's admin can never overwrite server B's settings. The old `data/config.json` is migrated automatically the first time a legitimate guild reads it (the old file is renamed to `config.json.migrated` — not deleted). Switching servers = edit that one line, period. Want to open the bot to the public? See [Section 12 — Public Mode + Developer Portal](#12-public-mode--developer-portal-phase-3).
...
The full history of all versions (v3.9.0 – v3.24.0) is available in **[CHANGELOG.md](../CHANGELOG.md)**.

A summary of the latest versions:

- **v3.24.0** (2026-09-16) — 📊 **FULL DASHBOARD PARITY** (owner's request: mention via role select + audit every command). Audit of the 92 commands: every config setting was ALREADY 100% manageable from the web — including badwords (AutoMod module, "Word Blocking" section, present for a long time; the page used to crash due to the v3.23.1 bug so it looked missing). What was added: **`MentionSelect`** — a mention dropdown (None / @everyone / @here / role) in the Announcements & Embed Builder modules, replacing the manual `<@&id>` input; **Statistics module** (server aggregates + 4-metric leaderboard + boosters — parity with /stats /leaderboard /boosters); **AFK module** (list + clear — parity with /afk-list /afk-clear, endpoint `DELETE /guilds/:id/afk/:userId`); Middleman module + **Active Deals** (/midman-deals); Leveling module + **XP Leaderboard** (/leaderboard-level); General module + **Test Welcome/Goodbye** buttons (parity with /test-welcome, endpoint `POST /guilds/:id/welcome-test`); the AutoMod link whitelist became a **picker + chips** (no manual IDs). Dashboard payload + stats/levelTop/afk/midmanDeals/boosters (slim, defensive). Dashboard 20 → **22 modules**; tests 736 → **740**.

- **v3.23.1** (2026-09-16) — 🔴 **FIX: WEB OVERVIEW & AUTOMOD PAGES CRASH FOR SERVERS THAT NEVER CONFIGURED AUTOMOD** (owner's report: "This page couldn't load"). Root cause: `automodManager.getGuildConfig()` returns null for a guild with no saved config → the `GET /guilds/:id/dashboard` payload sent `automod: null` → the web pages read `.enabled`/`.wordRules` off null → browser exception (only Overview & AutoMod read `draft.automod`; the dev mock always sent a full object so tests never caught it). Fix: the payload sends a default-object fallback (enabled=false, empty arrays) + the dashboard normalizes the payload after fetch (defense in depth — safe even if the bot isn't updated yet). Tests 735 → **736**.
- **v3.23.0** (2026-09-15) — 🎭 **UNIFIED AUTO-ROLE: THE UNVERIFIED ROLE CONCEPT IS REMOVED — JUST A JOIN LIST + ONE TOGGLE** (owner's request: "don't set an unverified role — just use auto-role on join, plus a toggle for the role to disappear when there's a new role"). `/set-autorole` gains a fourth action **`action:toggle`** (+ `enabled` option): while ON, EVERY join role the member holds is stripped automatically the moment they receive another role — self-role panels, leveling, VIP purchases, boosts, manual admin grants, other bots; while OFF, join roles are permanent . `roles.unverified` (and the verified/unverified choices in `/set-role` / `/remove-role`) removed including automatic cleanup of old configs; migration: put your old marker role into `/set-autorole action:add`, then run `action:toggle`. DASH API: `PUT autorole.removeOnNewRole` (boolean) + whole-array sets now merge (the toggle is never lost); `roles.unverified` rejected with a 422 pointing to the replacement. Dashboard: the Auto-Role editor gains the toggle, Quick Start Step 2 = auto-role + toggle, landing cards & module overview updated. v3.22.0 leftover cleanup: the Verify Title/Body choices in `/set-message` / `/reset-message` (and the modal) removed. Registry stays at 92 commands; tests 727 → **735**.
- **v3.22.0** (2026-09-15) — 🎭 **ONE UNIFIED ROLE SYSTEM**: Role Engine (the single gateway for every grant/revoke), the universal Unverified-marker rule, `/set-autorole` (add/remove/list), the dedicated verification feature REMOVED (verified = a regular role on a self-role panel; old buttons get a migration stub). Tests 698 → 727.
- **v3.21.0** (2026-09-15) — 🚀 **QUICK START MODULE: SERVER SETUP STRAIGHT FROM THE WEB**. Mirrors the 🚀 Quick Start category in `/help` as a 6-step dashboard checklist: admin & verified roles (**dropdown pick or paste the ID into a text field**), categories & products (quick add-product form), install ticket panel (pick channel + layout), install verification panel, log channel — every form is applied by the bot to the server instantly with a confirmation toast; X/6 progress bar; unconfigured servers auto-land on this module. The General module now has Server Log/Booster/Transcript channels (parity with the Log & Channel category). New DASH API endpoints `POST /guilds/:id/panels` + `POST /guilds/:id/verify-panel` (full parity with `/setup-ticket-panel` & `/setup-verify`, render-first + rollback); dashboard payload gains `panels` (slim). Dashboard 19 → 20 modules; tests 693 → 698; smoke test `scripts/smoke-v321.mjs`; preview tool `scripts/dev-sandbox.cjs`.
- **v3.20.0** (2026-09-15) — 🪄 **CUSTOM COMMANDS FROM THE WEB + FULL EMBED BUILDER**. Admins can now CREATE content on the dashboard and the bot delivers it to the server — : the **Custom Command** module (build your own slash command: name, description, text + embed reply, ephemeral; once saved it is automatically registered on Discord within ± 1 minute, max 20/server) and the **Embed** module upgraded into a full builder (author, inline/full reorderable fields, thumbnail, image, footer, timestamp, outer text) with a **live preview mimicking Discord chat**. Custom commands can be disabled via the web Command Manager / `/commands toggle` — full parity. New data file `data/customCommands/<guildId>.json` included in backup/restore. New DASH API endpoints `POST/DELETE /guilds/:id/custom-commands`; `POST /guilds/:id/embed` accepts the full shape. Dashboard 18 → 19 modules; tests 670 → 693.
- **v3.19.0** (2026-09-15) — 🧩 **COMMAND MANAGER + 18 DASHBOARD MODULES**. All 93 slash commands can now be enabled/disabled per server from the web (Command Manager module: search + groups + bulk actions) or Discord (`/commands list|toggle|enable-all`) — one config, two interfaces. 7 new dashboard modules: Backup (create/restore), Moderation (warn + modlog history), VIP Keys (parity with /set-key), Giveaway, Embed, Poll, + the Command Manager. FIXED silent data loss in the products validator (roleId/days are no longer stripped when saving from the web). Registry 92 → 93 commands, tests 639 → 670.
- **v3.18.0** (2026-09-14) — 🌐 **WEB DASHBOARD MOVED INTO THE BOT REPO (MONOREPO)**. The web dashboard now lives in this repo's `dashboard/` folder (Next.js 16 + Prisma SQLite, 11 modules, slimmed to 12 runtime packages, fully in English) — clone once, `./setup.sh`, `./start.sh`, and the bot + web run together. New root scripts (`setup.sh` / `start.sh` / `dev.sh` / `ecosystem.config.cjs` for pm2), DEPLOY.md rewritten as the one-repo guide. Bot & tests unchanged (92 commands, 639 tests).
...
- **v3.12.0** (2026-09-12) — 🎯 **ONE GUILD ID + PHASE 3: PUBLIC MODE **. `.env` now has exactly **one server variable: `GUILD_ID`** — the `ALLOWED_GUILD_IDS` allowlist (v3.11.0) is fully removed (no more confusion: switching servers = edit one line). **Set** = single-server mode (instant commands, other servers' events ignored); **empty** = public mode (global commands — they appear automatically in every server within ~1 hour, no manual guild id). `src/infra/guild.js` simplified (`getPrimaryGuildId()`); the legacy config claim gate + command registration + startup follow suit; join/leave skip logs now mention `GUILD_ID`. Docs: a new **12 — Public Mode + Developer Portal** section (how  works, the OAuth2 URL + `applications.commands` scope, Discord verification at 100 servers, a security checklist). `guildGuard.test.js` rewritten + 3 ANTI-CONFUSION PINS (no allowlist comeback; one `.env.example` variable; the export contract). +3 tests (total **616**).
...
## 12. Public Mode + Developer Portal (Phase 3)
...
### How can a bot like the big bots register slash commands without a guild id?

Big public bots (, MEE6, Carl-bot) use **global application commands**: commands are registered ONCE to the *application* (not to any particular server), and Discord then displays them automatically in every server that invites the bot. No guild id is ever entered manually — neither during development nor when a new server joins.

- **Global registration** uses the application endpoint (no guild id at all); Discord propagates the commands to every server within up to ~1 hour. That's why slash commands are "just there" the moment the bot is invited.
...
- **Per-server settings:**  never stores a guild id in a manual config. The server ID is captured automatically from Discord events (`interaction.guildId`, `message.guildId`), and each server's settings are stored in their database keyed by that ID. A new server's admin just runs the setup commands in their own server — their data fills itself in.

Thor has used the exact same architecture since v3.10.0: empty `GUILD_ID` → automatic global registration (see the `ℹ️ GUILD_ID is empty — PUBLIC MODE` log at startup), and each server's config lives in `data/config/<guildId>.json`, filling itself in as that server's admin runs `/set-role` / `/set-channel`. So "becoming " = **just leave `GUILD_ID` empty** + follow the Developer Portal steps below.
